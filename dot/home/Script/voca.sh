...
function count_occurrences() {
  count=$(grep -E "^$cleaned_text(\s\([0-9]+\))?$" ~/Downloads/data/obsidian/voca.md | wc -l)
  echo "$count"
}

# Check if the last copied text is the same as any word in voca.md
if grep -qE "^$cleaned_text(\s\([0-9]+\))?$" ~/Downloads/data/obsidian/voca.md; then
  ...
  echo "$cleaned_text ($count)" >> ~/Downloads/data/obsidian/voca.md
else
  # Append the cleaned text to voca.md if it does not exist
  echo "$cleaned_text" >> ~/Downloads/data/obsidian/voca.md

  # Remove unwanted characters and trailing white spaces from voca.md
  sed -i 's/[.\/?!"'\'':;]\s*$//g' ~/Downloads/data/obsidian/voca.md
fi
